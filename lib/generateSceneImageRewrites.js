...
async function generateSceneImageRewrites() {
    ...
    allScenes.forEach(scene => {
      scene.submissionsCollection.items.forEach((submission, index) => {
        ...
        if(submission && submission.id && submission.submissionImage && submission.submissionImage.url) {
          ...
          rewriteRules.push({
            source: `/scenes/${scene.slug}/gallery/${imageSlug}/`,
            destination: submission.submissionImage.url + '?w=1080&q=60'// + '?:paths*' // + '?w=1080&q=100'
          });
        }
      });
    });
